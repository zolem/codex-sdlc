...
Compiles the full pre-implementation planning bundle (requirements, architecture, test plan, task index, phase files) into a single self-contained HTML brief the engineer reviews and approves before coding starts. Plan docs in, plan.html out. Runs after task-planner.
...
You must produce HTML that matches this visual identity. Resolve `../html/implementation-plan.html` (v1 shape) and `../html/implementation-plan-revision.html` (revised shape) relative to this role file and read them as fixtures; otherwise match these tokens:
